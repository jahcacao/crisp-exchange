...
impl Contract {
    ...
    pub fn create_deposit(&mut self, asset: &AccountId, amount: U128) {
        ...
        let mut reserve = self.reserves.get(&asset).expect("Reserve not found");
    ...
    pub fn supply_collateral_and_borrow_leveraged(
        ...
        let mut reserve = self.reserves.get(&token0).expect("Reserve not found");
        reserve.borrowed += borrowed0;
        assert!(reserve.deposited >= reserve.borrowed);
        self.reserves.insert(&token0, &reserve);

        let mut reserve = self.reserves.get(&token1).expect("Reserve not found");
    ...
    pub fn return_collateral_and_repay(&mut self, borrow_id: u128) {
        let account_id = env::predecessor_account_id();
        let borrow = self.borrows.remove(&borrow_id).expect("Borrow not found");
        let pool = &self.pools[borrow.pool_id];
        let position = pool
            .positions
            .get(&borrow.position_id)
            .expect("Position not found");
        ...
        if let Some(leverage) = borrow.leverage {
            let mut reserve = self.reserves.get(&borrow.asset).expect("Reserve not found");
    ...
    pub fn get_borrow_health_factor(&self, borrow_id: BorrowId) -> f64 {
        let borrow = self.borrows.get(&borrow_id).expect("Borrow not found");
        let pool = &self.pools[borrow.pool_id];
        let position = pool.positions.get(&borrow.position_id).unwrap();
